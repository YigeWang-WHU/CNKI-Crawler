...
class SearchTools(object):
    ...
    def search_reference(self, ueser_input):
        ...
        # get请求中需要传入第一个检索条件的值
        key_value = quote(ueser_input.get('txt_1_value1', 'au_1_value1'))
    ...
    def pre_parse_page(self, page_source):
        ...
        is_all_record = input('是否要全部记录（y/n）?')
        # 将所有数量根据每页20计算多少页
        if is_all_record == 'y':
            ...
            select_record_num = int(input('请输入需要记录的数量：'))
            while True:
                if select_record_num > reference_num_int:
                    print('输入数量大于检索结果，请重新输入！')
                    select_record_num = int(input('请输入需要下载的数量（不满一页将下载整页）：'))
                else:
                    page, i = divmod(select_record_num, 20)
                    # 不满一页的记录一整页
                    if i != 0:
                        page += 1
                    print("开始记录前%d页所有文件，预计用时%s" % (page, s2h(page * 20 * 5)))
                    print('－－－－－－－－－－－－－－－－－－－－－－－－－－')
                    return page

    def parse_page(self, page_left, page_source):
        '''
        保存页面信息
        解析每一页的下载地址
        '''
        ...
        if page_left > 1:
            self.cur_page_num += 1
            self.get_another_page(page_left)

    def get_another_page(self, page_left):
        '''
        请求其他页面和请求第一个页面形式不同
        重新构造请求
        '''
        ...
        page_left -= 1
        self.parse_page(page_left, get_res.text)
